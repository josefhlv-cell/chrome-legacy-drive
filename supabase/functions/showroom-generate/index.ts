...
const PROMPT = [
  ...
  "1) NEVER mirror, flip, rotate, turn around, re-angle, or change the photographed side of the car. Preserve the exact left/right orientation from the FIRST image. If the source shows the left front corner, the output must still show the left front corner. If the source front points right, it must still point right; if it points left, it must still point left. Do not swap headlights, mirrors, wheels, doors, badges, plate orientation, or the visible side.",
  "2) Keep the vehicle itself 1:1 identical to the source photo except background removal and scale/position. No repainting, redesign, new wheels, new lights, changed trim, or AI re-rendered body shape.",
  "PLACEMENT RULES — match the reference placement style:",
  "3) Output canvas is exactly 1920x1080. The car must appear CLOSER TO THE CAMERA than before: large, low, and grounded on the asphalt. Do NOT make it small or high on the back wall.",
  "4) Target car bounding box: approximately x=160 to x=1880, bottom tire contact line y=1025–1040, roof around y=300–380 for normal minivans/cars. This makes the car fill about 88–90% of image width, like the reference placement, with only 40–55 px under the tires.",
  "5) The visual center of the car should be slightly right of canvas center, around x=1030–1080. The rear bumper/rear wheel must be visibly forward on the asphalt floor, not visually attached to the wall seam. There must be asphalt visible under the whole rear half of the vehicle.",
  "6) ALL visible tires/wheels must touch the asphalt floor on the same y=1025–1040 baseline. Wheels must NEVER touch or overlap the wall, the logo, the ceiling, the columns or any vertical surface.",
  "7) The floor-wall seam must stay BEHIND the car, while the tires sit BELOW that seam on asphalt. If any rear corner appears to sit on the wall, scale the car larger and move it down until the rear wheels clearly stand on asphalt.",
  "8) The roof, bumpers, mirrors and all wheels must remain fully visible. Never crop the car. Never make the car tiny. Never place the car in the middle of the wall.",
